...
{%- set name = item.name[:-1] -%}
{%- set no = item.name[-1:] -%}
  Type rollershutter : rollershutter{{loop.index}}	"{{item.label}}"	[ stateTopic="stat/{{topic}}/RESULT", transformationPattern="JSONPATH:$.{{name}}{{item.label}}{{no}}", commandTopic="cmnd/{{topic}}/{{name}}{{item.label}}{{no}}" ]
{%- endfor %}
{%- for item in string %}
...
{%- for item in color %}
Color {{label}}_{{item.name}}_Color		"{{item.label}}"	{{item.icon}}	({{item.groups}})	{{item.tags.split(',')}}	{ channel="mqtt:topic:{{thingid}}:color{{loop.index}}" {{ item.metadata }} }
{%- endfor %}
{%- for item in contact %}
Contact {{label}}_{{item.name}}{{loop.index}}		"{{item.label}}"	{{item.icon}}	({{item.groups}})	{{item.tags.split(',')}}	{ channel="mqtt:topic:{{thingid}}:contact{{loop.index}}" {{ item.metadata }} }
{%- endfor %}
{%- for item in dimmer %}
Dimmer {{label}}_{{item.name}}_Dimmer		"{{item.label}}"	{{item.icon}}	({{item.groups}})	{{item.tags.split(',')}}	{ channel="mqtt:topic:{{thingid}}:dimmer{{loop.index}}" {{ item.metadata }} }
{%- if 'ct' in features %}
...
{%- endfor %}
{%- for item in number %}
Number {{label}}_{{item.name}}_{{item.label|replace(" ", "_")}}		"{{item.label}}"	{{item.icon}}	({{item.groups}})	{{item.tags.split(',')}}	{channel="mqtt:topic:{{thingid}}:{{item.name}}_{{item.label|replace(" ", "_")}}" {{item.metadata}}}
{%- endfor %}
{%- for item in player %}
{%- endfor %}
{%- for item in rollershutter %}
Rollershutter {{label}}_{{item.name}}_{{item.label|replace(" ", "_")}}		"{{item.label}}"	{{item.icon}}	({{item.groups}})	{{item.tags.split(',')}}	{channel="mqtt:topic:{{thingid}}:rollershutter{{loop.index}}" {{item.metadata}}}
{%- endfor %}
{%- for item in string %}
String {{label}}_{{item.name}}_{{item.label|replace(" ", "_")}}		"{{item.label}}"	{{item.icon}}	({{item.groups}})	{{item.tags.split(',')}}	{channel="mqtt:topic:{{thingid}}:{{item.name}}_{{item.label|replace(" ", "_")}}" {{item.metadata}}}
{%- endfor %}
{%- for item in switch %}
...
{% set groups = groups|default([]) -%}
{% set tags = tags|default([]) -%}
Switch {{label}}_{{item.name}}_{{item.label|replace(" ", "_")}}		"{{item.label}}"	{{item.icon}}	({{item.groups}})	{{item.tags.split(',')}}	{channel="mqtt:topic:{{thingid}}:power{{loop.index}}" {{item.metadata}}}
{%- endfor %}
